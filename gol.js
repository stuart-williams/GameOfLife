const columnContainsLiveCells = (cells, n) => cells.map((row) => row[n]).includes(1)
...
function trim (cells) {
  ...
  const newCells = cells.reduce((accum, row, i) => {
    if (!columnContainsLiveCells(cells, 0)) row = row.slice(1)
    if (!columnContainsLiveCells(cells, rowEnd)) row = row.slice(0, -1)
    if ((i === 0 || i === cellsEnd) && !row.includes(1)) return accum
    return [ ...accum, row ]
  }, [])

  if (
    !columnContainsLiveCells(newCells, 0) ||
    !columnContainsLiveCells(newCells, newCells[0].length - 1) ||
    !newCells[0].includes(1) ||
    !newCells[newCells.length - 1].includes(1)
  ) {
